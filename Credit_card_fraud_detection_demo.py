# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 21:43:25 2020
...
df = df.sample(frac=0.05, random_state = 4949)
...
if st.sidebar.checkbox('Show fraud and valid transaction details'):
    st.write('Number of Fraudulent Transactions: %.3f%%'%outlier_percentage)
...
size = st.sidebar.slider('Test Set Size', min_value=0.1, max_value=0.3)
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = size, random_state = 4949)#Print shape of train and test sets
...
etree=ExtraTreesClassifier(random_state=4949)
rforest=RandomForestClassifier(random_state=4949)
